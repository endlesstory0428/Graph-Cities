...
var greyColor = new THREE.Color(0, 0, 0);
...
G.addModule("view", {
        ...
        if (this.model && this.model.nodes) {
            ...
            if (layouts) {
                ...
                if ((!newGraph.vertices.layout) && (newGraph.datasetID == this.subviews[0].graph.datasetID)) {
                    ...
                    let oldLayout = layouts[0], oldIDs = this.subviews[0].graph.vertices.id,
    ...
    displayGraph: async function (graph, options) {
        if (!options) options = {};
        if (graph.vertices.layout) for (let i = 0; i < graph.vertices.layout.length; i++) {
    ...
    sharedUniforms: {
        ...
        edgeList: {
            isArray: true,
            value: (model) => {
                let links = model.links, linkSources = links.source, linkTargets = links.target,
                    linkStrengths = links.strength, linkDistances = links.distance;
                let result = [];
        ...
        layerHeights: {
            ...
            value: (model) => {//assuming they are non negative, scale to between -0.5 and 0.5
                let max = model.heights.max, min = model.heights.min, layerCount = Object.keys(model.heights).length,
    ...
    templates: {
        nodes: {
            ...
            properties: {
                ...
                adjlist: {
                    isArray: true, isAttribute: false,//for building some uniforms
                    value: (model) => {
                        let adjlist = [];
                ...
                colorIndex: {
                    ...
                    value: (model) => {
                        ...
                        return linkColorValues.map((v) => {
                            if (v == -1) return -1; else return model.colorIndexMap[v]
                        });//-1 means interpolate
                    }
                },
        ...
        lines: {
            ...
            properties: {
                position: {
                    dimensions: 3, value: (line, i, array) => {
                        var v = new THREE.Vector3();
                        v.x = array.source[i];
                        v.y = array.target[i];
                        return v;
                ...
                coord: {
                    dimensions: 3, value: quadCoordFunc, perPoint: true,
                },
            },
            pointsPerObject: 6,
            uniforms: {
        ...
        waveLayers: {
            ...
            getObjectAtPos: function (pos) {
                ...
                let buffer = G.view.getPositions(), vec = new THREE.Vector3(), tolerance = 0.03;//ratio of the screen size
                let p1 = new THREE.Vector2(), p2 = new THREE.Vector2(), p3 = new THREE.Vector2(),
                    p4 = new THREE.Vector2(), point = new THREE.Vector2().copy(pos);
                let nodePos = new THREE.Vector3(), eye = new THREE.Vector3(), horizontal = new THREE.Vector3(),
                ...
                for (let i = 0; i < waveLayers.length; i++) {
                    ...
                    vertical.copy(horizontal).cross(worldUp).normalize().multiplyScalar(0.5 * shape.y);
        ...
        waveArcs: {
            ...
            getObjectAtPos: function (pos) {
                ...
                for (let i = 0; i < arcs.length; i++) {
                    let maxRadius = arcs.maxRadius[i], minRadius = maxRadius - arcs.thickness[i],
                        centerAngle = arcs.centerAngle[i], angleWidth = arcs.angleWidth[i];
    ...
    refreshAttrs: function (updateAll) {//now, it creates attrs directly from properties marked as such, turning arrays of vectors into typed arrays using the dimensions property.
        ...
        for (let name in this.templates) {
            let obj = this.templates[name];
            let data = model[name], length = data.length;//should only be created once - todo: attach as a module to graph objects?
            ...
            if (subviewObjTemplate) for (let attrName in subviewObjTemplate.properties) {
    ...
    updateUniform(template, uniformsObj, uniformName, args) {
        ...
        if (template.isArray) {
            let uniformSizeName = uniformName + "Size";
            let newSizeObj = {value: template.sizeCache};
            uniformsObj[uniformSizeName] = newSizeObj;
            let uniformCountName = uniformName + "Count";
            let newCountObj = {value: template.countCache};
