...
G.addModule("view",{
	...
	init:function(){
		...
		d3.text("shaders/sharedShaderLib.vs").then((data)=>{if(data){this.shaderLib=data;}});
		d3.text("shaders/simulation.vs").then((data)=>{if(data){this.shaderSources["simulation.vs"]=data;}});
		d3.text("shaders/simulation.fs").then((data)=>{if(data){this.shaderSources["simulation.fs"]=data;}});
		for(let name in this.templates){
			if(!this.templates[name].object3dType)continue;
			d3.text("shaders/"+name+".vs").then((data)=>{if(data){this.shaderSources[name+".vs"]=data;}});
			d3.text("shaders/"+name+".fs").then((data)=>{if(data){this.shaderSources[name+".fs"]=data;}});
		}
		G.zoomOutDistance=()=>{
	...
	displayGraph:async function(graph){
		...
		if(heightPropertyName){
			if(heightPropertyName in graph.edges.properties)heightPropertyType="edges";//??!!
		...
		if(graph.vertices.length>10000){G.simulationRunning=false;}
	...
	sharedUniforms:{
		...
		nodeColorData:{
			isArray:true,
			value:(model)=>{
				let nodes=model.nodes;let colorIndices=nodes.colorIndex;
				let result=nodes.map((node,i,array)=>{
					return {x:colorIndices[i],y:0,z:0,w:0};
				});
				return result;
			}
		},
		...
		nodeTargetRadiusData:{
			isArray:true,
			value:(model)=>{
				let nodes=model.nodes;
				let result=nodes.map((node,i,array)=>{
					return {x:array.targetRadius[i],y:0,z:0,w:0};
				});
				return result;
			}
		},
		...
		linkLayerColorRatio:{value:()=>G.controls.get("linkLayerColorRatio",0.75),dynamic:true},
		lineLayerColorRatio:{value:()=>G.controls.get("lineLayerColorRatio"),dynamic:true},
		
		colorList:{
			isArray:true,
			value:(model)=>model.colorList,
		},
		
		layerColors:{
			isArray:true,
			value:(model)=>{
				let logColorScale=G.controls.get("logColorScale");
				let size=Math.ceil(Math.sqrt(model.heights.max+1));//starts from 0
				let layerColors=[];let layerCount=model.heights.count;if(layerCount==0)layerCount=1;
				//let saturation=(G.showingSparseNet||G.graph.showingInterlayers)?1:((layerCount>10)?1:(0.01+0.99*Math.sqrt(layerCount-1)/3));
				let saturation=(layerCount>1)?0.9:0.01;if(G.brightColors)saturation=1;
				...
				if(logColorScale){
					...
					for(let l in model.heights){let diff=l-lastl;lastl=l;let temp=(Math.log(Math.log(diff+1.01)+1.01));logSum+=temp;logs[l]=logSum;}
					for(let l in model.heights){layerColors[l]=new THREE.Color();layerColors[l].setStyle(colorScale(logs[l]/(logSum)));let hsl=layerColors[l].getHSL();layerColors[l].setHSL(hsl.h,saturation,nodeL);}
				}
				else{
					for(let l in model.heights){layerColors[l]=new THREE.Color();layerColors[l].setStyle(colorScale(l/(model.heights.max+1)));let hsl=layerColors[l].getHSL();layerColors[l].setHSL(hsl.h,saturation,nodeL);}
				}
				let realParent=G.loading.getParent(G.graph);//get the relevant metagraph or original graph, skip single-metanode ancestors - todo: how does height color  inheritance work under the hierarchy view? How about if the previously shwn metagraph is not the natural metagraph (say layer CC meatgraph)?
				if(realParent&&realParent.layerColors&&G.controls.get("inheritColors",true)){
					let followParent=true;
					for(let l in model.heights){if(l in realParent.heights==false){followParent=false;break;}}//parent's layer colors is continuous from 0 to max  but heght may not be; need to test against heights
					if(model.showingInterlayers){followParent=false;}
					if(followParent){for(let l in model.heights){layerColors[l]=realParent.layerColors[l].clone();}}
				}
				for(let i=0;i<layerColors.length;i++){if(!layerColors[i])layerColors[i]=new THREE.Color();}
				G.graph.layerColors=layerColors;//hack to follow parent layer colors
				G.graph.heights=model.heights;//hack to follow parent layer colors
				return layerColors;
		...
		layerHeights:{
			isArray:true,
			value:(model)=>{
				let max=model.heights.max,logLayerHeightRatio=((model.showingInterlayers||(model.layerHeightOption=="linear"))?0:G.controls.get("logLayerHeightRatio")),layerCount=Object.keys(model.heights).length;
				...
				for(let i=0;i<=max;i++){
					...
					//if(model.showingInterlayers)temp=1-temp;
		...
		nodes:{
			...
			properties:{
				...
				colorIndex:{
					isArray:true,
					value:(model)=>{
						let nodes=model.nodes,colorValues=nodes.colorValue,linkColorValues=model.links.colorValue;//todo:should make it work for all objects?
						let colors={},colorIndexMap={},colorList=[];//colorList will be a texture
						let colorIndices=[];let tempScale=colorScale;//G.colorScale
						if(G.view.graph.colorScale){tempScale=G.colorScales[G.view.graph.colorScale];}
						colorValues.forEach((value,i,array)=>{
							if(value in colors==false){
								let c=new THREE.Color();if(value==-1){c.copy(greyColor);}else {c.setStyle(tempScale(value));}
								colors[value]=c;
								let index=colorList.length;colorList.push(c);colorIndexMap[value]=index;
							}
						});
						linkColorValues.forEach((value,i,array)=>{
							if(value in colors==false){
								let c=new THREE.Color();if(value==-1){c.copy(greyColor);}else {c.setStyle(tempScale(value));}
								colors[value]=c;
								let index=colorList.length;colorList.push(c);colorIndexMap[value]=index;
						...
						model.colorList=colorList;model.colorIndexMap=colorIndexMap;
						return colorValues.map((v)=>colorIndexMap[v]);
					}
				},
		...
		heights:{
			...
			value:(model)=>{
				...
				if(model.nodes.length==0){
					...
					for(let l in G.graph.layers){heights[l]=G.graph.layers[l];}
				}
				else{
					let nodeHeights=model.nodes.height;
					nodeHeights.forEach((height,index)=>{
						...
						heights[height].v++;heights[height].nodes.push(model.nodes[index]);
					});
					let linkSources=model.links.source,linkTargets=model.links.target;
