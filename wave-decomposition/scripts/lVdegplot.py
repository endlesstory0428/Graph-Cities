...
fig.update_layout(
    ...
    scene_aspectratio=dict(x=3, y=2),
...
fig.add_trace(
    go.Scatter(
        x=lineaxis, y=lowerline, mode='lines', line_color='rgba(0,0,0,1)', name=''
    )
)
fig.add_trace(
    go.Scatter(
        x=lineaxis, y=upperline, mode='lines', line_color='rgba(0,0,0,1)', name=''
    )
)
