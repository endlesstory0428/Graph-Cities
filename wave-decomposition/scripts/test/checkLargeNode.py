...
with open(f'{graph}/cityMesh/SPIRAL.txt') as fr:
	with open(f'{graph}/cityMesh/SPIRALDAG.txt', 'w') as fw:
		for line1, line2 in itertools.zip_longest(fr, fr, fillvalue = ''):
			...
			if int(info[12]) == 0:
				building2dagType[(int(fp), int(lcc))] = 'wcc.dag'
			elif int(info[12]) == 1:
				building2dagType[(int(fp), int(lcc))] = 'edgeCut.dag'
			else:
				building2dagType[(int(fp), int(lcc))] = 'wf.dag'
...
with open(f'{graph}/mallVicinityList.json', 'w') as f:
	json.dump([(*info, building2dagType[(info[0], info[1])]) for info in mallVicinityList], f)
