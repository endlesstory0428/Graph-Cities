...
void readGraph(const std::string &inputFile, unsigned int numedges, bool dounion, unsigned int maxLabel)
{
	rank = std::vector<unsigned int>(maxLabel + 1);
	parent = std::vector<unsigned int>(maxLabel + 1);
	ds = new boost::disjoint_sets<unsigned int *, unsigned int *>(&rank[0], &parent[0]);
	verts = boost::dynamic_bitset<>(maxLabel + 1);
	if (dounion)
		numedges += numedges;
...
int main(int argc, char *argv[])
{
	...
	unsigned int maxLabel = 0;
	if (argc > 4) {
		maxLabel = atol(argv[4]);
	} else {
		maxLabel = atol(argv[2]) * (dounion ? 2 : 1);
	}

	std::string prefix = argv[1];
	reset();
	readGraph(prefix, atol(argv[2]), dounion, maxLabel);
	if (DEBUG)
