...
void fillInput(string inputFile) {
    ...
    while (getline(is, line)) {
        ...
        numLayers = max(numLayers, layer);

        if (input.size() <= layer) {
            input.resize(layer*2);
            layerLcc2Idx.resize(layer*2);
        }
        if (input[layer].size() <= lcc) {
            input[layer].resize(lcc*2);
            layerLcc2Idx[layer].resize(lcc*2);
        }
        input[layer][lcc] = true;
        layerLcc2Idx[layer][lcc] = idx;
...
void readCcLayers(string layerFile) {
...
void updateCounts(string folder) {
    ...
    for (int layer = 1; layer <= numLayers; layer++) {
        ...
        while (getline(is, line)) {
            ...
            if (input.size() <= layer || input[layer].size() <= v2lcc[v2idx[v]][layer] || !input[layer][v2lcc[v2idx[v]][layer]]) continue;
            ui layerLccIdx = layerLcc2Idx[layer][v2lcc[v2idx[v]][layer]];
...
void writeMetaData(vector<long long> &timeList) {
    ofstream out("info.json");
...
int main(int argc, char *argv[]) {
    ...
    v2lcc.resize(g.NODENUM+1, vector<ui>(input.size()));
    for (string ccLayerFile : ccLayerFiles) readCcLayers(graphLayerPath + ccLayerFile);
    timeList.push_back(getTimeElapsed());
    cout << "READ LCC\n";
    reset();
