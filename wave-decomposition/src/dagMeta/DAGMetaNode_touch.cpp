...
#define TH_LARGENODE ((unsigned int)1 << 18)
    ...
    v_size_type &verts)
{
    ...
    vector<v_size_type> node2out_e(local2v.size());

    o.open(output_file_prefix + ".vmap");
    // std::cout << output_file_prefix + ".vmap" << "\n";
    for (int i = 1; i < local2v.size(); i++)
    {
    ...
    for (int i = 0; i < node2set.size(); i++)
    {
        // cout << "node_num_v" << i << "," << node2num_v.size() << "\n";
        if (node2num_v[i] == 0)
            continue;
        ++ nodeNum;
        // cout << "node2set" << i << "," << node2set.size() << "\n";
        vLocal = node2set[i];
        // cout << "local2v" << vLocal << "," << local2v.size() << "\n";
        maxNodeLabel = max(maxNodeLabel, local2v[vLocal]);
        // cout << "local2wave" << vLocal << "," << local2wave.size() << "\n";
        // cout << "local2frag" << vLocal << "," << local2frag.size() << "\n";
        // cout << "wf2nodeNum" << local2wave[vLocal] << "," << wf2nodeNum.size() << "\n";
        // cout << "wf2nodeNum-f" << local2frag[vLocal] << "," << wf2nodeNum[local2wave[vLocal]].size() << "\n";
        ++ wf2nodeNum[local2wave[vLocal]][local2frag[vLocal]];
        // cout << "local2frag" << vLocal << "," << local2frag.size() << "\n";
        // cout << "node_num_e" << i << "," << node2num_e.size() << "\n";
        // cout << "node_num_e" << vLocal << "," << node2out_e.size() << "\n";
        o << local2v[vLocal] << ","
          << local2wave[vLocal] << ","
          << local2frag[vLocal] << ","
          << local2level[vLocal] << ","
          ...
          << node2num_e[i].load() << ","
          << node2out_e[vLocal] << "\n";
        if (node2num_e[i].load() + node2out_e[vLocal] > TH_LARGENODE) {
            largeNodes.push_back(i);
        }
        // string v_str = to_string(local2v[node2set[i]]);
        // nodes["nodes"][v_str]["set"] = local2level[node2set[i]];
        // nodes["nodes"][v_str]["wave"] = local2wave[node2set[i]];
...
void newFPMetaDagCover(string g, string outPath, v_size_type l, v_size_type lcc, int nt, v_size_type max_v)
{
    ...
    vector<label_size_type> local2v(1);
    vector<wf_type> local2wf(1);
    vector<v_size_type> local2wave(1), local2frag(1);
    fill_local2wave_frag(v2local, local2v, local2wf, wave_sources, local2wave, local2frag);
    cout << "size:" << local2v.size() << "," << local2v[0] << "," << local2v[1] << "\n";
    timeList.push_back(getTimeElapsed());
