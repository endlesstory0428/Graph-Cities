...
def logBucket(sizeDict, totalSize):
	...
	baseThreshold = math.log(math.log(totalSize / 2)) 
	thresholdList = [0]
	bucketLength = 1
	while True:
...
if __name__ == '__main__':
	...
	print(lcc2Bucket)
	with open(f'{graph}-lccBuck.l-lcc-b.csv', 'w', newline = '') as f:
		csvWriter = csv.writer(f)
		for (l, lcc), bucket in lcc2Bucket.items():
			csvWriter.writerow((l, lcc, bucket))
	...
	for bucket, bucketInfo in lccBucketInfo.items():
		if 1 in bucketInfo['peel']:
			...
			if count > 1:
				...
				bucketInfo['peel'][1]['lccList'] = [{'layer': 1, 'count': count, 'vertices': edgeCount + count, 'edges': edgeCount, 'single': False}]
			else:
	...
	with open(f'{graph}-lccWaves.vBuck.b.p.mm.json', 'w') as f:
