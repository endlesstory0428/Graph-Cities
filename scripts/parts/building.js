...
function createFlags(scene, height, coord, base_Y, layer, V, E, fragNum, fragNeg, fragPos, fragBucket, duplicate, flag_objects, lcc, peel, mast_scale, dataSet, flag_objects_new, glyphInfo) {
...
function createCityMeshes(scene, objects, city_all, city_tracking, ceil_objects, middle_objects, truss_objects, window_objects, flag_objects, flag_objects_new, arrow_objects, src_objects, tgt_objects, city_to_load, y_scale, dataSet, ceilVisible, isNight, oneBuilding = false) {
  for (let layer in city_tracking) {
    ...
    if (city_tracking[layer].ready_to_move && city_tracking[layer].ready_to_color) {
      ...
      let result = createFlags(scene, height - 1, [X, Z], flag_base_Y, layer, city_all[layer].V, city_all[layer].E, city_all[layer].fragNum, city_all[layer].fragNeg, city_all[layer].fragPos, city_all[layer].fragBucket, city_all[layer].duplicate, flag_objects, lcc, fixed, mast_scale, dataSet, flag_objects_new, city_all.glyphInfo[layerShortName]);
      scene = result.scene;
