...
print('Creating metaids')
id2lcc = df.groupby(['l', 'cc']).size().reset_index().drop(0, axis=1)
lcc2id = id2lcc.groupby(['l', 'cc']).groups
print('Created metaids')
...
metaedges = pd.DataFrame.from_dict(medges, orient='index', columns=['w'])
