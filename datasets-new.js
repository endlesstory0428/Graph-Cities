...
const Graph=require("./lib/graph-new.js"),Algs=require("./lib/graph-algorithms.js"),fs = require('node-fs'),zlib = require('zlib'),Buffer = require('buffer').Buffer,path = require('path'),readline=require('readline'),{exec, spawn} = require('child_process');
var datasetFiles={};
var datasets={};
var datasetList;
var datasetIDMaps={};
...
function getDatasetSummary(d){
	let result={};
	for(let p of summaryProperties){if(p in d)result[p]=copyObj(d[p]);}
	for(let p in summaryFunctions){let v=summaryFunctions[p](d);if(v!==undefined)result[p]=copyObj(v);}
	return result;
}
function getDatasetList(){return datasetList;}
...
function loadAllDatasets(d,c,callback,failure,cachesToRefresh,noLoad){
	dataDir=d,cacheDir=c;
	if(!fileExists("")){mkdir("");}
	checkFilesInDataDir(dataDir);
	loadAllFiles(datasetFiles,cachesToRefresh,noLoad).then(async(v)=>{
		makeDatasetList();
		if(callback)callback(datasetList);
		console.log("***loading all datasets finished*** "+(v||""));
		loadExtraData();//await ?
	}).catch((e)=>{console.log(e);console.log("***loading all datasets failed***");failure();});
}
...
function reloadAllDatasets(cachesToRefresh){
	...
	loadAllFiles(datasetFiles,cachesToRefresh).then(async(v)=>{
		...
		console.log("***loading all datasets finished*** "+(v||""));
		loadExtraData();//await ?
	}).catch((e)=>{console.log(e);console.log("***loading all datasets failed***");});
}
...
function checkDatasetTopLevel(filepath,topLevelFileName){
	...
	if(stat.isDirectory()==false){
		...
		let id=segments.slice(0,segments.length-1).join("."),type=segments[segments.length-1];if(id=="")id=topLevelFileName;
		if(!datasetFiles[id])datasetFiles[id]=[];
		datasetFiles[id].push({type:type,fileName:topLevelFileName,path:filepath,updateTime:stat.mtimeMs,size:stat.size});
	}
	else {
...
function checkAllFilesInDataset(filepath,fileName,datasetID){
	...
	if(stat.isDirectory()==false){
		...
		datasetFiles[datasetID].push({type:type,path:filepath,fileName:fileName,updateTime:stat.mtimeMs,size:stat.size});
	}
	else {
	...
	"cit-Patents":{
		...
		load:()=>{
			return new Promise((resolve,reject)=>{
				...
				if(!cattxt){console.log("missing "+catfile);reject();return;}
				...
				lineReader.on('line', function (line) {
					...
					map[patentID]={id:patentID,year:Number(columns[1]),country:columns[4],state:columns[5],subcat:subcats[columns[11]]};
					
				});
				lineReader.on('close', ()=>{console.log("loaded data "+filename);resolve({map:map,subcats:subcats});});
			});
		},
		get:function (ids){
...
let DataTemplates={//these functions all take a graph(that has a dataPath etc) not a whole dataset.
	...
	graph:{ //no deps because it exists when the graph is created (top level graphs will have this generated if needed automatically)
		...
		make:(g)=>{saveAllProperties(g);},//if the raw data contains other properties like weight, also save them? 
		//such properties have no make method if they are treated as templates
		load:(g)=>{loadTopology(g);},
	},
	ccID:{//of the top level graph
		...
		condition:(g)=>{//layers can have ccs and those subgraphs have its own subgraphs
			...
			for(let p of g.partitionInfo){if(p.type=="wave"||p.type=="waveLevel"||p.type=="level")return false;}//?? //||p.type=="wave2" is allowed
	...
	fixedPointLayer:{
		deps:"graph",condition:(g)=>{
			if(g.dataPath==g.datasetID||(g.partitionInfo.length==1&&g.partitionInfo[0].type=="CC"))return true;return false;
		},
	...
	waveSubgraphs:{//separated from wave calculation because saving many waves can be very annoying, and we want wave maps but not wave subgraphs for very large graphs
		deps:"graph,waveAndWaveLevel",condition:(g)=>{
			...
			if(g.partitionInfo)for(let p of g.partitionInfo){if(p.type=="wave"||p.type=="wave2"||p.type=="waveLevel"||p.type=="level")return false;}
			return true;
		},
	...
	cc:{//separated from ccID because this crashing would mess up the summary
		...
		make:(g)=>{
			let ccids=g.vertices.cc;if(!ccids)throw Error();
			//also save subgraphs
	...
	layerCC:{//calculates and saves ccIDs and cc subgraphs for all layers, and use these objects in memory to produce the global layerCC metagraph (there are no global layerCC IDs, instead vertices in this metagraph refer to a relatve data path like layer/* /CC/ * so it can load the correct graph)
		deps:"graph,layer",files:["metagraphs/layerCC"],condition:(g)=>{if(g.dataPath==g.datasetID||(g.partitionInfo.length==1&&g.partitionInfo[0].type=="CC"))return true;return false;},
		make:(g)=>{
			...
			if(g.dataPath==g.datasetID&&g.vertices.properties.cc){
				if(g.vertices.properties.cc.isAbstract)loadProperty(g,"vertices","cc");originalCCIDs=g.vertices.cc;
			...
			let vertexCCMap=new Array(g.vertices.length);for(let i=0;i<vertexCCMap.length;i++){vertexCCMap[i]={};}
			let metagraph=new Graph();metagraph.originalGraph=g.dataPath;metagraph.vertices.addProperty("V");metagraph.vertices.addProperty("E");metagraph.vertices.addProperty("originalLayer");metagraph.edges.addProperty("E");metagraph.heightProperty="originalLayer";
			...
			for(let layer of list){
				...
				else{saveSubgraphs(layer,"CC",result.subgraphs);}
	...
	BCC:{//only CC subgraphs can have BCCs - In fact I think only CCs of a layer can have BCCs
		...
		condition:(g)=>{
			...
			for(let p of g.partitionInfo){
				...
				return false;}}//if at any level it's in layer 1, reject
	...
	waveCC:{
		...
		make:(g)=>{
			...
			let metagraph=new Graph();metagraph.originalGraph=g.dataPath;metagraph.vertices.addProperty("V");metagraph.vertices.addProperty("E");metagraph.vertices.addProperty("originalWave");metagraph.vertices.addProperty("isMetanode");//wave ccs tend to contain isolated vertices, so don't save them as subgraphs
			metagraph.edges.addProperty("E");metagraph.heightProperty="originalWave";
			let waveCCSummary={};
	...
	waveMap:{//may work for very large graphs, doesn't require wave subgraphs
		...
		make:(graph)=>{
			...
			let waveMap=[];let arcs=[];let arcLinks=[];
	...
	levelCC:{
		...
		make:(g)=>{
			...
			let vertexCCIDs=new Array(g.vertices.length);for(let i=0;i<vertexCCIDs.length;i++){vertexCCIDs[i]=-1;}
			let vertexLevelMarks=new Array(g.vertices.length);for(let i=0;i<vertexLevelMarks.length;i++){vertexLevelMarks[i]=-1;}//need to keep track of which levels we assigned stuff to
			...
			let metagraph=new Graph();metagraph.originalGraph=g.dataPath;metagraph.vertices.addProperty("V");metagraph.vertices.addProperty("E");metagraph.vertices.addProperty("originalWaveLevel");
			metagraph.edges.addProperty("E");metagraph.heightProperty="originalWaveLevel";
			let levelCCSummary={};
			...
			for(let level of list){
				...
				for (let ccID in result.subgraphs){
					...
					for(let vID of vertexList){
						let realLevel=levelIDs[vID];
						if(vertexLevelMarks[vID]==levelID){levelCCids[vID]=metanodeID;}//need to mark even vertices not in this level, but prefer to assign vertices to higher levels
	...
	levelMap:{//like waveMap, but for each wave subgraph's levels (skips bucketed waves)
		...
		make:(graph)=>{
			...
			let restrictedWaveLevel=new Int32Array(graph.vertices.length);restrictedWaveLevel.fill(-1);
			for(let vID=0;vID<graph.vertices.length;vID++){
				let levelID=waveLevels[vID];let levelCCid=levelCCids[vID];
				...
				for(let neighbor of graph.getNeighbors(vID)){
					...
					if(edgeLevel>restrictedWaveLevel[vID]){restrictedWaveLevel[vID]=edgeLevel;}
				}
			}
			
			
			let levelMap={};let arcs=[];let arcLinks=[];
			for(let vID=0;vID<graph.vertices.length;vID++){
				...
				for(let neighbor of graph.getNeighbors(vID)){
					...
					if(otherLevel>levelID){levelObj[levelCCid].ef++;}//count forward and internal edges
					if(otherLevel==levelID&&neighbor>vID){levelObj[levelCCid].e++;}
				}
			}
			let metagraph=new Graph();metagraph.levelMap=levelMap;//metagraph.arcLinks=arcLinks;
...
for(let name in DataTemplates){
	...
	if((typeof obj.optionalDeps)=="string"){obj.optionalDeps=obj.optionalDeps.split(",");}
	if(!obj.deps)obj.deps=[];if(!obj.optionalDeps)obj.optionalDeps=[];
	if((typeof obj.files)=="object"){let temp=obj.files;obj.files=()=>temp;}
}


async function loadAllFiles(datasetFiles,cachesToRefresh,noLoad){
	...
	for(let id in datasetFiles){
		...
		datasets[id]={id:id,data:new Graph()};let dataset=datasets[id];
		...
		if(!fs.existsSync(cacheDir+"/"+mainGraph.dataPath)){fs.mkdirSync(cacheDir+"/"+mainGraph.dataPath);}
		...
		if(!basicFilesExist){needLoading=true;}
		else if(basicFilesUpdateTime<dataNewestTime){needLoading=true;}
		if(!summaryFileExists){needLoading=true;}
		else if(summaryFileUpdateTime<dataNewestTime){needLoading=true;}
		...
		if(noLoad){//get top level summary only
		...
		while(graphStack.length>0){
			...
			while(graphStack.length>0){
				...
				if(!graph.savedData){graph.savedData={};}//note: only non-bucketed graphs can enter the stack, and at this point thye should ave their initial data saved already
				...
				while(hasRemainingTemplate){
					nextTemplate=null;hasRemainingTemplate=false;
							...
							if((!templateObj.make)&&dataExists){//it's an initial data that should exist
								...
								if(tempfilesExist){
									...
									if(updateTime<mainGraph.updateTime){
										...
										if(!templateObj.optional){throw Error("outdated required initial data "+name+" at "+graph.dataPath);}
									...
									if(!templateObj.optional){throw Error("missing required initial data "+name+" at "+graph.dataPath);}
								}
					...
					let needGenerating=false;let currentDataUpdateTime,newestFileName;
					...
					if((!needGenerating)&&templateObj.alwaysMake){needGenerating=true;
					...
					async function loadCache(name){//loading something doesn't require loading its dependencies
						...
						if(currentCacheTemplate.load){await Promise.resolve(currentCacheTemplate.load(graph));}//a cache file may have no separate loading method, and loading may or may not be async
					...
					if(needGenerating){
						...
						if(templateObj.deps){await Promise.all(templateObj.deps.map(loadCache));}
						if(templateObj.optionalDeps){
							await Promise.all(templateObj.optionalDeps.filter((name)=>graph.savedData[name]&&graph.savedData[name].exists!==false).map(loadCache));}//these don't have to exist but if they do they will be loaded, eg. to add optional data only available at some places on the hierarchy
						if(!templateObj.make){throw Error("no method to make required cache: "+nextTemplate);}
						let success=undefined;
						try{
							await Promise.resolve(templateObj.make(graph)).then(()=>{success=true;}).catch((e)=>{success=false;});
						}
						catch(e){
							console.log(e.stack);
							success=false;
				...
				if(!graph.subgraphs){graph.subgraphs={};}if(!graph.metagraphs){graph.metagraphs={};}
				for(let type in subgraphTypes){if(type in graph.subgraphs==false){
					//console.log("detected subgraphs "+type);
				derivedGraphsChanged=true;}}
				for(let type in graph.subgraphs){if(type in subgraphTypes==false){
					//console.log("detected missing subgraphs "+type);
				derivedGraphsChanged=true;}}
				let metagraphTypes=getMetagraphFileTypes(graph);
				for(let type in metagraphTypes){if(type in graph.metagraphs==false){
					//console.log("detected metagraph "+type);
				derivedGraphsChanged=true;}}
				for(let type in graph.metagraphs){if(type in metagraphTypes==false){
					//console.log("detected missing metagraph "+type);
				derivedGraphsChanged=true;}}
				if(derivedGraphsChanged){
					graph.subgraphs=subgraphTypes;graph.metagraphs=metagraphTypes;
			...
			if(stashedGraphs.length>0){//if no progress is made in this run through the stack, stop
				if(!savedDataChangedInStack){throw Error("cyclic dependency detected in stashed graphs");}
				else{
...
function getLoadingMethod(id,files){
	...
	if(!file)return null;;//other files we want to ignore
	if(file.size<10000000){
		...
		return ()=>{
			return new Promise(function(resolve, reject) {
				...
				lineReader.on('close', ()=>{console.log("loaded "+id+", file size:"+file.size+", read size: "+sizeRead+", lines: "+datasets[id].data.lineNumber);resolve();});
			});
...
function startStreaming(client,options){
	let datasetID=options.datasetID,filetype=options.filetype,chunkSize=options.chunkSize||256,interval=options.interval||1000;
	if((datasetID in datasets)==false){client.emit("warning","no such dataset "+datasetID+"/"+filetype);return;}
	let streams=client.streams;
	if(!streams[datasetID])streams[datasetID]={};
	if(streams[datasetID][filetype]){client.emit("warning","already streaming "+datasetID+"/"+filetype);return;}
	//now just load the array 
	try {var array=loadObj(datasetID,filetype);}
	catch(e){
		client.emit("warning","cannot load streaming file "+datasetID+"/"+filetype);return;
	}
	//console.log(array);
	...
	obj.streamNext=function(){
		while(1){
			...
			if(nextJSON.length+totalLength>chunkSize){
				if(objs.length>0){break;}//send the ones before
				else{objs.push(array[nextIndex]);nextIndex++;client.emit("warning","streamed data record larger than chunk size");break;}//this single object is somehow too large, but probably should send it anyway
			}
			else{objs.push(array[nextIndex]);nextIndex++;totalLength+=nextJSON.length+1;}
		}
		client.emit("stream",{datasetID:datasetID,type:filetype,data:objs});console.log("streamed "+objs.length+" records");
		objs=[];totalLength=1;
	};
	...
	obj.pause=function(){
		if(obj.interval){clearInterval(streams[datasetID][filetype].interval);delete streams[datasetID][filetype].interval;}
		else{console.log("cannot pause an inactive stream "+datasetID+"/"+filetype);}
	}
	obj.stop=function(){
		if(obj.interval){clearInterval(streams[datasetID][filetype].interval);delete streams[datasetID][filetype].interval;}
		else{console.log("trying to stop an inactive stream "+datasetID+"/"+filetype);}
		if(this==streams[datasetID][filetype])delete streams[datasetID][filetype];
	}
	obj.resume=function(){
		if(!obj.interval){obj.interval=setInterval(obj.streamNext,interval);}
		else{console.log("cannot resume an active stream "+datasetID+"/"+filetype);}
	}
}
...
function stopStreaming(client,options){
	...
	let streams=client.streams;
	if(!streams[datasetID])streams[datasetID]={};
	if(streams[datasetID][filetype]){clearInterval(streams[datasetID][filetype]);delete streams[datasetID][filetype];}
	else{client.emit("warning","not currently streaming "+datasetID+"/"+filetype);}
}
function pauseStreaming(client,options){
	...
	let streams=client.streams;
	if(!streams[datasetID])streams[datasetID]={};
	if(streams[datasetID][filetype]){streams[datasetID][filetype].pause();client.emit("warning","paused streaming "+datasetID+"/"+filetype);}
	else{client.emit("warning","not currently streaming "+datasetID+"/"+filetype);}
}
function resumeStreaming(client,options){
	...
	let streams=client.streams;
	if(!streams[datasetID])streams[datasetID]={};
	if(streams[datasetID][filetype]){streams[datasetID][filetype].resume();}
	else{client.emit("warning","not currently streaming "+datasetID+"/"+filetype);}
}
function stopAllStreaming(client){
	...
	for(let datasetID in streams){
		...
		if(streams[datasetID][filetype]){clearInterval(streams[datasetID][filetype]);delete streams[datasetID][filetype];}
		else{client.emit("warning","not currently streaming "+datasetID+"/"+filetype);delete streams[datasetID][filetype];}
	}
}
	...
	switch (type){
		case "sparsenet":
			return new Promise((resolve,reject)=>{
				...
				let exePath="bin/sparsenet";if(options&&options.variant){exePath+="_"+options.variant;}
				try{
					...
					sn.on('close', (code) => {
						...
						if(!result.trim()){console.log("invalid sn for custom graph: ",result);reject();return;}//sometimes it may crash
		...
		default: console.log("unknown computation type "+type);
	}
}
...
function newestFileOfFiles(files,dir){
	...
	for(let f of files){
		let filePath=cacheDir+"/"+(dir||"")+"/"+f,stat=fs.statSync(filePath);
		if (stat.mtimeMs>newest){newest=stat.mtimeMs;newestFileName=f;}
	}
	return newestFileName;
}

function fileExists(filename){return fs.existsSync(cacheDir+"/"+Array.from(arguments).join("/"))}
function fileStat(filename){return fs.statSync(cacheDir+"/"+Array.from(arguments).join("/"))}
function readdir(filename){return fs.readdirSync(cacheDir+"/"+Array.from(arguments).join("/"))}
function mkdir(filename){
	fs.mkdirSync(cacheDir+"/"+Array.from(arguments).join("/"),511,true);
}
function unlink(filename){fs.unlinkSync(cacheDir+"/"+Array.from(arguments).join("/"))}
function rmdir(filename){fs.rmdirSync(cacheDir+"/"+Array.from(arguments).join("/"))}
...
function loadObj(){
	...
	var str=zlib.gunzipSync(zip);return JSON.parse(str);
}
...
function loadArrayWithProperties(){//loads the same json file as loadObj, but interpret it as an array with hidden array properties, by treating obj.value as the original array, and attaching all things in obj.properties on it as hidden properties.
	...
	let result=obj.value; if(!Array.isArray(value))throw Error("not an array");
...
function saveObj(){
	...
	let str;if(isTypedArray(obj))str="["+obj.join(",")+"]";else str=JSON.stringify(obj);
...
function toJSON(obj){//custom conversion for eg. objects containing typed arrays
	...
	switch(typeof obj){
		case "object":
			...
			if(isTypedArray(obj)){return "["+obj.join(",")+"]";}
...
//returns a list of subgraphs (unloaded) that are not bucketed.
	...
	let bucketVLimit=16384;let bucketELimit=65536;
	...
	let partitionInfo=options.partitionInfo;if(!partitionInfo){partitionInfo={graph:g.dataPath};}//by default the base graph is the parent graph
	...
	let bucketsSummary=[],bucketMap={},bucketItemCount=0,bucketVertexCount=0,bucketEdgeCount=0,bucketIndex=0;
	...
	if(!subgraphs[Symbol.iterator]){subgraphs=Object.keys(subgraphs).sort(compareBy((x)=>Number(x),true)).map((key)=>subgraphs[key]);}//if it's an object, sort the items by ascending values (to be the same order as arrays)
	...
	for(let newGraph of subgraphs){//this works even if result is a generator function(to save memory)
		if(!newGraph){console.log("missing subgraph");continue;}
		if(options.skipSingleVertexSubgraphs&&(newGraph.vertices.length==1)){continue;}
		subgraphCount++;
		let subgraphSummary=newGraph.getShortSummary();
		let subgraphID=newGraph.subgraphID;subgraphSummary.subgraphID=subgraphID;
		...
		let subgraphPartitionInfo=Object.assign({},partitionInfo);subgraphPartitionInfo.type=type;subgraphPartitionInfo.value=subgraphID;
		...
		if(g.globalPartitionInfo){for(let name in g.globalPartitionInfo){newGraph.globalPartitionInfo=g.globalPartitionInfo[name];}}
		if(g.dataPath==g.datasetID){newGraph.globalPartitionInfo[type]={value:subgraphID,V:newGraph.vertices.length,E:newGraph.edges.length};}
		...
		if(!Vdist[V])Vdist[V]=0;Vdist[V]++;if(V>unbucketedVLimit){if(!VdistIDs[V])VdistIDs[V]=[];VdistIDs[V].push(subgraphID);}
		if(!Edist[E])Edist[E]=0;Edist[E]++;if(E>unbucketedELimit){if(!EdistIDs[E])EdistIDs[E]=[];EdistIDs[E].push(subgraphID);}
		if(!VEdist[VE])VEdist[VE]=0;VEdist[VE]++;if(V>unbucketedVLimit||E>unbucketedELimit){if(!VEdistIDs[VE])VEdistIDs[VE]=[];VEdistIDs[VE].push(subgraphID);}
		...
		if((!options.noBucketing)&&((options.bucketCondition&&(options.bucketCondition(newGraph)))||((newGraph.vertices.length<=unbucketedVLimit)&&(newGraph.edges.length<=unbucketedELimit)))){
			...
			if(newGraph.vertices.length+bucketVertexCount>bucketVLimit||newGraph.edges.length+bucketEdgeCount>bucketELimit){
				...
				let keys=Object.keys(bucketMap).map((x)=>Number(x)),max=Math.max.apply(null,keys),min=Math.min.apply(null,keys);
				bucketsSummary.push({index:bucketIndex,max:max,min:min});
				saveObjStr(g.dataPath,type,"buckets",bucketIndex,toJSON(bucketMap));
				bucketMap={},bucketItemCount=0,bucketVertexCount=0,bucketEdgeCount=0;bucketIndex++;
			}
			subgraphSummary.bucketed=true;subgraphSummary.bucketID=bucketIndex;newGraph.bucketID=bucketIndex;
			bucketItemCount++;bucketVertexCount+=newGraph.vertices.length;bucketEdgeCount+=newGraph.edges.length;
		...
		if(!fileExists(g.dataPath,type,subgraphID)){mkdir(g.dataPath,type,subgraphID);}
	...
	if(bucketItemCount>0){
		//console.log("saving bucket of "+bucketItemCount+" items");
		let max=0,min=Infinity;for(let key in bucketMap){let num=Number(key);if(num>max)max=num;if(num<min)min=num;}
		bucketsSummary.push({index:bucketIndex,max:max,min:min});
		let str=toJSON(bucketMap);if(typeof str!="string"){console.log(bucketMap);console.log(str);throw Error();}
		saveObjStr(g.dataPath,type,"buckets",bucketIndex,str);
	}
	
	if(!g.subgraphs)g.subgraphs={};
	let simpleSummary={count:subgraphCount,max:maxSubgraphID,min:minSubgraphID,buckets:bucketsSummary,Vdist:Vdist,Edist:Edist,VEdist:VEdist,VdistIDs:VdistIDs,EdistIDs:EdistIDs,VEdistIDs:VEdistIDs,unbucketed:unbucketedList.map((g)=>g.subgraphID)}
...
function loadSubgraphSummary(g,type){//summary.json.gz
	let dataPath=g;if(typeof g=="object")dataPath=g.dataPath;
...
function loadSubgraphs(g,type,includeBucketed=false){
	...
	let subgraphs=[];let bucketsFound=false;
...
function getBuckets(originalGraph,subgraphType){
	...
	bucketsCache=buckets;bucketsCachePath=originalGraph+"/"+subgraphType;
	return buckets;
}
...
function getBucket(originalGraph,subgraphType,bucketID){
	...
	let bucket=result;
	bucketCache=bucket;bucketCachePath=originalGraph+"/"+subgraphType+"/buckets/"+bucketID;
	return bucket;
}
...
function toNormalText(str){//un-Camel case
	str=String(str);
	return str.replace(/(?<=[a-z])([A-Z]+)/g, ' $1').replace("_"," ").replace(/^./, function(str){ return str.toUpperCase(); });
}
