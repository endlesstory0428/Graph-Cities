	...
	for (long i = 0; i < currTail; i++) {
		...
		if(g.start_indices[v] == 0 && g.end_indices[v] == 0) {
			...
			for (unsigned int j = g.start_indices[v]; j <= g.end_indices[v]; j++) {
				if(edgeLabels[j] < buk && edgeLabels[j] > 1) {
...
void labelEdgesAndUpdateDegree(unsigned int peel, unsigned int *core, unsigned int *degree, unsigned int *edgeLabels) {
	for(unsigned int i = 0; i < g.EDGENUM; i++) {
		unsigned int src = (g.edgeList + i)->src;
		unsigned int tgt = (g.edgeList + i)->tgt;

		if (edgeLabels[i] == peel || edgeLabels[i] == 0)
...
int main(int argc, char *argv[]) {
	...
	std::fill_n(edgeLabels, g.EDGENUM, 0);
	...
	do {
		...
		for(unsigned int i = 0; i <= g.NODENUM; i++) {
			...
			if(core[i] > mc2 && core[i] < mc)
				mc2 = core[i];
		}
		labelEdgesAndUpdateDegree(mc, core, degree, edgeLabels);
		if(DEBUG)
			std::cout<<"CURRENT MAXIMUM CORE : "<<mc<<", "<<mc2<<"\n";
	...
	t.join();
	writeLayerToFile(argv[1], 1, originalIndices, edgeLabels, node2label);
