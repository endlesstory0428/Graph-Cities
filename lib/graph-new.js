...
class Graph extends DataSet{
	constructor(){
		...
		//this.vertices=[];addHiddenProperty(this.vertices,"id",[]);addHiddenProperty(this.vertices,"edges",[]);this.vertexMap={};
		//this.edges=[];addHiddenProperty(this.edges,"source",[]);addHiddenProperty(this.edges,"target",[]);
		this.warningCount=0;this.warningReasons={};this.repeatedWarningLimit=3;
	}
	...
	addEdge(source,target){
		...
		if(this.hasVertex(target)==false){if(this.warnAboutUnknownVertex){this.warn(source,"created unknown vertex");}this.addVertex(target);}
		...
		for(let graphID=0;graphID<subgraphs.length;graphID++){
			...
			for(let i=0;i<vertices.length;i++){
				...
				if(id in vertexMap){
					...
					index=graph.addVertex(id);vertexMap[id]=index;
					for(let propName in vertices.properties){
						if(i in vertices[propName]){graph.vertices[propName][index]=vertices[propName][i];}
					}
					if(subgraphIDPropertyName){
	...
	loadVertices(text){//for strings only. if it's an array, just set this.vertices and this.vertices.id and use updateGraph when needed
		var g=this;
	...
	//actally we skip weight for now
		...
		for(let line of edgelistLines){this.loadEdgeLine(line);}//avoid repeating similar code
		return this;
	}
