	...
	static getFilteredSubgraph(g,propertyName,targetValue,propertyOwner, vertices=null){//or a property array can be given instead of propertyName
		...
		if(propertyOwner=="vertices"){
            ...
            if (!vertices) {
                ...
                subgraph.initData = {}
                subgraph.addVertex(G.view.graph.vertices.id[propertyName].toString())
                initData.nodes.push({id: G.view.graph.vertices.id[propertyName].toString(), label: G.view.graph.labelsByID[propertyName][G.controls.getLabelIndex()]})
                for (let j in vertices) {
                        subgraph.addVertex(vertices[j])
                        subgraph.addEdge(G.view.graph.vertices.id[propertyName].toString(), vertices[j]);
                        initData.nodes.push({id: vertices[j], label: G.view.graph.labelsByID[G.view.graph.vertices.id.indexOf(vertices[j])][G.controls.getLabelIndex()]})
                        initData.links.push({
                            source: G.view.graph.vertices.id[propertyName].toString(),
                            target: vertices[j]
                        })
                }
                subgraph.initData = initData;
