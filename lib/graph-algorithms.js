//let _Graph;
//if(typeof Graph=="undefined")_Graph=require("./graph-new.js");
//else _Graph=Graph;

if(typeof Graph=="undefined")Graph=require("./graph-new.js");
class GraphAlgorithms{
	...
	static getVertexPartition(g,partition){
		...
		for(let value in subgraphs){
			if(subgraphs[value].vertices.length==1){
		...
		for(let value in subgraphs){
			if(subgraphs[value].vertices.length==1)continue;
	...
	static getEdgePartition(g,partition){
		//returns all subgraphs, and a metagraph, and the partition summary etc
		...
		for(let l in layers){
			let metanodeID=metagraph.addVertex(l);let subgraph=layers[l];
