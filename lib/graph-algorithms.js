	...
	static getFilteredSubgraph(g,propertyName,targetValue,propertyOwner){//or a property array can be given instead of propertyName
		...
		if(propertyOwner=="vertices"){
            ...
            subgraph.initData = initData;
                    ...
                    subgraph.initData={};
                    for(let j in getProperty(subgraph.vertices,propertyName,"edges")){
                            subgraph.addVertex(j)
                            initData.nodes.push(
                                {id: j,
                                 label: G.view.graph.labelsByID && G.view.graph.labelsByID[j]?G.view.graph.labelsByID[j].name:j
                                }
                                )
    ...
    static getAvgNumOfConnectionsByLabel(g, label) {
	    ...
	    if(label == "places"){
            for (let i =0; i< g.vertices.id.length; i++) {
                if(g.labelsByID[i] && (g.labelsByID[i][0].split(" ")).length == 1){
            ...
            for (let i =0; i< g.vertices.id.length; i++) {
                if(g.labelsByID[i] &&((g.labelsByID[i][0].split(" ")).length >= 2 && !g.labelsByID[i][0].includes("ETK") && !g.labelsByID[i][0].includes("ATU") && !g.labelsByID[i][0].includes("TMI") )){
                    if(g.snnumConnections[i]>0) {
                        count = count +1;
                        sum = sum + g.snnumConnections[i];
                    }
            ...
            for (let i =0; i< g.vertices.id.length; i++) {
                if(g.labelsByID[i] && g.labelsByID[i][0].includes(label)){
                    if(g.snnumConnections[i]>0) {
                        count = count +1;
                        sum = sum + g.snnumConnections[i];
                    }
	    ...
	    for(let i = 0; i<=sequence; i++){
	        ...
	        if(!G.view.graph.story || !G.view.graph.story[thisPath]) {
                if (G.view.graph.snPaths && !G.snNodesColorByLabel) {
                    ...
                    if (G.view.graph.labelsByID[start] && G.view.graph.labelsByID[end] && !G.view.graph.labelsByID[start][0].includes("ETK") && G.view.graph.labelsByID[end][0].includes("ETK")) {
                        let temp = end;
                        end = start;
                        start = temp;
                    }
                    //let r1 = G.view.graph.labelsByID[start].match(/[A-Z][0-9]{1,9}/);
                    //let r2 = G.view.graph.labelsByID[start].match(/[A-Z] # [0-9]{1,9}/);
                    if (G.view.graph.labelsByID[start] && G.view.graph.labelsByID[start][0] && G.view.graph.labelsByID[start][0].includes("ETK")) {
                        let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3} [0-9]{1,9}:/);
                        let storyName = "";
                        if (r1 && r1[0] != undefined) {
                            storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
                        } else storyName = G.view.graph.labelsByID[start][0];
                        story += storyName;
                        storyAll.push(storyName)
                    } else if (G.view.graph.labelsByID[start] && G.view.graph.labelsByID[start][0] && G.view.graph.labelsByID[start][0].includes("ATU")) {
                        let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3} # [0-9]{1,9}/);
                        if (r1 && r1[0] != undefined) {
                            let storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
                            story += storyName;
                            storyAll.push(storyName);
                        } else {
                            let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3} #/);
                            let storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
                            story += storyName;
                            storyAll.push(storyName);
                        }
                    } else if (G.view.graph.labelsByID[start] &&  G.view.graph.labelsByID[start][0] && G.view.graph.labelsByID[start][0].includes("TMI")) {
                        let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3}[0-9]{1,9}/);
                        let storyName = "";
                        if (r1 && r1[0] != undefined) {
                            storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
                    ...
                    while (start != end) {
                        ...
                        for (let worm in worms) {
                            if (worms[worm].includes("ETK")) {
                                let r1 = worms[worm].match(/[A-Z]{1,3} [0-9]{1,9}:/);
                                let storyName = "";
                                if (r1 && r1[0] != undefined) {
                                    storyName = worms[worm].substring(r1[0].length);
                                } else storyName = worms[worm];
                                if (storyAll.indexOf(storyName) == -1) {
                                    formattedWorms.push(storyName);
                                    storyAll.push(storyName);
                                }

                            } else if (worms[worm].includes("ATU")) {
                                let r1 = worms[worm].match(/[A-Z]{1,3} # [0-9]{1,9}/);
                                if (r1 && r1[0] != undefined) {
                                    let storyName = worms[worm].substring(r1[0].length);
                                    if (storyAll.indexOf(storyName) == -1) {
                                        formattedWorms.push(storyName);
                                        storyAll.push(storyName);
                                    }
                                } else {
                                    let r1 = worms[worm].match(/[A-Z]{1,3} #/);
                                    let storyName = "";
                                    if (r1 && r1[0] != undefined) {
                                        storyName = worms[worm].substring(r1[0].length);
                                    } else storyName = worms[worm];
                            ...
                            } else if (worms[worm].includes("TMI")) {
                                let r1 = worms[worm].match(/[A-Z]{1,3}[0-9]{1,9}/);
                                let storyName = "";
                                if (r1 && r1[0] != undefined) {
                                    storyName = worms[worm].substring(r1[0].length);
                        ...
                        discovered.push(start);
                        if(G.view.graph.labelsByID[start] && G.view.graph.labelsByID[start][0]) {
                            if (G.view.graph.labelsByID[start][0].includes("ETK")) {
                                let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3} [0-9]{1,9}:/);
                                let storyName = "";
                                if (r1 && r1[0] != undefined) {
                                    storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
                            ...
                            } else if (G.view.graph.labelsByID[start][0].includes("ATU")) {
                                let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3} # [0-9]{1,9}/);
                                if (r1 && r1[0] != undefined) {
                                    let storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
                                    if (storyAll.indexOf(storyName) == -1) {
                            ...
                            } else if (G.view.graph.labelsByID[start][0].includes("TMI")) {
                                let r1 = G.view.graph.labelsByID[start][0].match(/[A-Z]{1,3}[0-9]{1,9}/);
                                let storyName = "";
                                if (r1 && r1[0] != undefined) {
                                    storyName = G.view.graph.labelsByID[start][0].substring(r1[0].length);
