        ...
        for (let i = 0; i < G.view.graph.thePaths.length; i++) {
            ...
            if (G.view.graph.snPaths && !G.snNodesColorByLabel) {
                ...
                if (G.view.graph.labelsByID[start] && G.view.graph.labelsByID[start][0]) {
                    storyName = this.clearText(G.view.graph.labelsByID[start][0]);
                ...
                while (count != G.view.graph.thePaths.length) {
                    ...
                    if (sharedNeighbors && Object.keys(sharedNeighbors).length > 0) {
                        ...
                        // let allhotspots = Object.keys(G.view.graph.hotspotsPathsMapping);
                        // let sharedHotspots = (allhotspots).filter(function (item) {
                        //     return firstPath.includes(item);
                        // });
                        // for(let i =0; i<sharedHotspots.length; i++){
                        //     storyName = this.clearText(G.view.graph.labelsByID[sharedHotspots[i]][0]);
                        //     if (storyAll.indexOf(storyName) == -1) {
                        //         story += "<span style='color: red'>" + storyName + "</span>";
                        //         storyAll.push(storyName);
                        //         let wormsIds = G.view.graph.snwormsListIds[sharedHotspots[i]];
                        //         let worms = G.view.graph.snwormsList[sharedHotspots[i]];
                        //         let formattedWorms = [];
                        //         for (let worm in worms) {
                        //             if (worms[worm]) {
                        //                 storyName = this.clearText(worms[worm]);
