...
class GraphAlgorithms{
    ...
    static getStoriesByHotspot(){
	    let paths = G.view.graph.thePaths;
	    let hotspotsIds = [];
                ...
                if (G.view.graph.snPaths && !G.snNodesColorByLabel) {
                    ...
                    while (count != G.view.graph.thePaths.length) {
                        ...
                        for (let i = 0; i < neighTrack.length; i++) {
                            if (hotspotsIds.indexOf(neighTrack[i]) != -1 && G.view.graph.thePaths.indexOf(G.view.graph.hotspotsPathsMapping[neighTrack[i]]) && neighTrack[i] != start && discovered.indexOf(neighTrack[i]) == -1) {
