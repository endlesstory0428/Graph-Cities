...
else if ( event.keyCode === 17) { _state = "control";}

 if ( event.keyCode === 87 || event.keyCode === 40 ) { _keyPanEnd.y+=0.05;} //up w
 if ( event.keyCode === 83 || event.keyCode === 38 ) { _keyPanEnd.y-=0.05;} //down s
 if ( event.keyCode === 65 || event.keyCode === 39) { _keyPanEnd.x+=0.05;} //left a
 if ( event.keyCode === 68 || event.keyCode === 37) { _keyPanEnd.x-=0.05;} //right d
...
else if ( event.keyCode === 80) {_state = STATE.PAN;}
...
function contextmenu( event ) {
	...
	this.setTarget=function(obj, minus=false){
